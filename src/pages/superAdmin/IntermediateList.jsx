...
function IntermediateList() {
  ...
  const [currentIntermediate, setCurrentIntermediate] = useState(null);
    ...
    const fetchIntermediates = async () => {
      try {
        ...
        const response = await fetch(
          `${import.meta.env.VITE_SITE}/superadmin/intermediates`,
          {
            headers: {
        ...
        setError(`Failed to fetch intermediates: ${err.message}`);
        console.error(err);
      } finally {
        setLoading(false);
      }
  ...
  useEffect(() => {
    let results = intermediates;
    if (searchTerm) {
      const lowercasedSearch = searchTerm.toLowerCase();
      results = results.filter(
        (intermediate) =>
          intermediate.fullName.toLowerCase().includes(lowercasedSearch) ||
          intermediate.username.toLowerCase().includes(lowercasedSearch) ||
          intermediate.email.toLowerCase().includes(lowercasedSearch) ||
          intermediate.role.toLowerCase().includes(lowercasedSearch)
      );
    }
    ...
    if (filterStatus !== "all") {
      ...
      results = results.filter(
        (intermediate) => intermediate.isActive === isActive
      );
    }

    if (filterDepartment !== "all") {
      results = results.filter(
        (intermediate) => intermediate.department === filterDepartment
      );
    }

    setFilteredIntermediates(results);
  }, [searchTerm, filterStatus, filterDepartment, intermediates, reload]);
  ...
  const handleEdit = (intermediate) => {
    setError(null);
    setIsEditing(true);
    setCurrentIntermediate(intermediate);
    setFormData({
      ...
      contactNumber: intermediate.contactNumber || "",
  ...
  const handleSubmit = async (e) => {
    ...
    if (isEditing) {
      url = `${import.meta.env.VITE_SITE}/superadmin/intermediates/${
        currentIntermediate._id
      }`;
      method = "PUT";
    } else {
      url = `${import.meta.env.VITE_SITE}/superadmin/intermediates`;
    ...
    try {
      ...
      // Update the Intermediates list
      if (isEditing) {
        setIntermediates((prevIntermediates) =>
          prevIntermediates.map((intermediate) =>
            intermediate._id === currentIntermediate._id ? data : intermediate
          )
        );
      } else {
        setIntermediates((prevIntermediates) => [
        ...
        setReload(!reload);
        if (
          data.intermediate.department &&
          !departments.includes(data.intermediate.department)
        ) {
          setDepartments([...departments, data.intermediate.department]);
        }
      }
  ...
  // Handle intermediate deletion
  const handleDelete = async (intermediateId) => {
    if (!window.confirm("Are you sure you want to delete this intermediate?")) {
      return;
    }
    ...
    try {
      ...
      const response = await fetch(
        ...
        }/superadmin/intermediates/${intermediateId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      // Remove intermediate from state
      setIntermediates((prevIntermediates) =>
        prevIntermediates.filter(
          (intermediate) => intermediate._id !== intermediateId
        )
      );
    } catch (err) {
      setShowForm(false);
      setError(`Failed to delete intermediate: ${err.message}`);
      console.error(err);
    }
  };

  // Toggle intermediate active status
  const handleToggleActive = async (intermediate) => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `${import.meta.env.VITE_SITE}/superadmin/intermediates/${
      ...
      const updatedIntermediate = await response.json();

      // Update intermediate in state
      setIntermediates((prevIntermediates) =>
        prevIntermediates.map((a) =>
          a._id === intermediate._id ? updatedIntermediate : a
        )
      );
    } catch (err) {
      setShowForm(false);
      setError(`Failed to update intermediate status: ${err.message}`);
      console.error(err);
    }
  };
    ...
    <div className="max-w-[1200px] mx-auto sm:px-4 p-2 py-8 min-h-[80vh]">
      ...
      <div className="mb-6 bg-gray-800 rounded-lg p-4 shadow-md">
        ...
        <div className="hidden sm:flex flex-wrap gap-4">
          ...
          <div className="ml-auto flex items-center text-gray-300 text-sm">
            <span>
              {filteredIntermediates.length}{" "}
              {filteredIntermediates.length === 1
                ? "intermediate"
                : "intermediates"}{" "}
              found
            </span>
      ...
      {intermediates.length === 0 ? (
        <div className="bg-gray-800 text-gray-200 rounded-lg shadow-md p-6 text-center">
          <p>
            No intermediates found. Create a new intermediate to get started.
          </p>
        </div>
      ) : filteredIntermediates.length === 0 ? (
        <div className="bg-gray-800 text-gray-200 rounded-lg shadow-md p-6 text-center">
          <p>No intermediates match your search or filter criteria.</p>
          <button
            onClick={resetFilters}
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-700">
              ...
              <tbody className="bg-gray-800 divide-y divide-gray-700">
                {filteredIntermediates.map((intermediate) => (
                  <motion.tr
                    key={intermediate._id}
                  ...
                  >
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex flex-col">
                        <div className="text-sm font-medium text-gray-200">
                          {intermediate.fullName}
                        </div>
                        <div className="sm:hidden text-xs text-gray-400">
                          {intermediate.username} • {intermediate.role}
                        </div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      >
                        {intermediate.isActive ? "Active" : "Inactive"}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex space-x-2">
                        <button
                          onClick={() => handleEdit(intermediate)}
                        ...
                        <button
                          onClick={() => handleToggleActive(intermediate)}
                          className={`${
                            intermediate.isActive
                          ...
                          title={
                            intermediate.isActive ? "Deactivate" : "Activate"
                          }
                        >
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                        ...
                        <button
                          onClick={() => handleDelete(intermediate._id)}
            ...
            >
              <div className="flex justify-between items-center border-b border-gray-700 px-6 py-4">
                <h3 className="text-xl font-semibold">
                  {isEditing ? "Edit Intermediate" : "Add New Intermediate"}
                </h3>
                <button
              ...
              <form onSubmit={handleSubmit} className="p-6">
                ...
                <div className="mt-6 flex justify-end space-x-3">
                  ...
                  >
                    {isEditing ? "Update Intermediate" : "Create Intermediate"}
                  </motion.button>
                </div>
              </form>
            </motion.div>
